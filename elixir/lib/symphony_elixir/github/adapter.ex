defmodule SymphonyElixir.GitHub.Adapter do
  ...
  def claim_issue(issue, metadata), do: client_module().claim_issue(issue, metadata)

  @spec update_workpad(term(), map()) :: :ok | {:error, term()}
  def update_workpad(issue, metadata), do: client_module().update_workpad(issue, metadata)
